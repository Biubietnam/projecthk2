...
export default function PetAdoptionPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const petRes = await axios.get(`https://thoriumstudio.xyz/api/pets/${id}`);
                setPet(petRes.data);

                const gearRes = await axios.get(`https://thoriumstudio.xyz/api/gears`);
                ...
                if (token) {
                    const adoptionRes = await axios.get(`https://thoriumstudio.xyz/api/adoption/check/${id}`, {
                        headers: {
