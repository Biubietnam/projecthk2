...
export default function ContentPetDetail({ pet }) {
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:8000/api/favorite/${pet.id}`, {
            ...
            toast.success(res.data.is_favorite ? 'This pet is in your favorites!' : 'This pet is not in your favorites.');
            console.log('Favorite pets fetched:', res.data);
        }).catch(err => {
            toast.error('Failed to fetch favorite status. Please try again later.');
            console.error('Error fetching favorites:', err);
        }).finally(() => {
            setLoading(false);
        });
    }, [pet.id, closeModal, navigate]);
    ...
    const toggleFavorite = (petId) => {
        ...
        axios.post(`http://localhost:8000/api/favorite/${petId}`, {}, {
            ...
            toast.success(`Pet ${petId} favorite status toggled.`);
            console.log(`Pet ${petId} favorite status toggled.`);
        }).catch(err => {
            toast.error('Failed to toggle favorite status. Please try again later.');
    ...
    return (
        <motion.div
            ...
            className="text-left text-sm w-full text-gray-700"
        >
                <Toaster
                    ...
                    <div className="w-full md:w-1/3 flex-shrink-0">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                            {Array.isArray(pet.tags) && pet.tags.map(tag => (
                                <span key={tag} className="inline-block bg-purple-100 text-purple-800 text-xs px-2 py-1 rounded mr-2 mb-2 font-medium">
                                    {tag}
                                </span>
                            ))}
                        </div>
  ...
  `}
                    ...
                    <Link to={`/pet/${pet.id}`} onClick={closeModal} aria-label="Go to adoption page" className="py-2 px-4 bg-customPurple text-white text-sm rounded-md hover:bg-customPurpleDark transition font-medium">
                        Adopt Me
                    </Link>
                </div>
            </motion.div>
